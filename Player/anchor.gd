...
func _physics_process(delta):
	if not player.scope:
		...
		if player.zoomed or player.scope:
			global_rotate(Vector3.UP, deg2rad(player.zoom_ang_speed * direction.x * delta))
			global_rotation.y = clamp(global_rotation.y, deg2rad(-30), deg2rad(30))
			rotate_object_local(Vector3(1, 0, 0), deg2rad(player.zoom_ang_speed * direction.y * delta))
